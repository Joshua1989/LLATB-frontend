...
def update_card_data():
    ...
    df_unit['is_promo'] = df_unit.apply(lambda x: x['smile_max'] > 1 and
                                        x['normal_icon_asset'] == x['rank_max_icon_asset'] and
                                        x['max_removable_skill_capacity'] == x['default_removable_skill_capacity'], axis=1)
    df_unit['is_limit'] = df_unit.apply(lambda x: x['smile_max'] > 1 and
                                        x['normal_icon_asset'] == x['rank_max_icon_asset'] and
                                        x['max_removable_skill_capacity'] > x['default_removable_skill_capacity'], axis=1)
